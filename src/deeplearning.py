...
class DeepLearning:
    ...
    def __make_layer(self, I, n_input, n_output, weight, bias, activ):
        ...
        if weight == 'zeros':
            ...
            W = tf.Variable(tf.random_normal([n_input, n_output], stddev=0.01))  # for FizzBuzz
        elif weight == 'truncated_normal':
            W = tf.Variable(tf.truncated_normal([n_input, n_output], stddev=0.01))
        
        # Bias
        if bias == 'zeros':
            B = tf.Variable(tf.zeros([n_output]))  # for FizzBuzz
        elif bias == 'ones':
            B = tf.Variable(tf.ones([n_output]))

        # Activation Function
        if activ == '':
            O = tf.matmul(I, W) + B  # for FizzBuzz
        elif activ == 'relu':
            O = tf.nn.relu(tf.matmul(I, W) + B)  # for FizzBuzz
        elif activ == 'tanh':
